...
async def rate_limit_if_no_bypass(request: Request):
    if get_rate_limit_key(request) != BYPASS_SECRET_KEY:
        return RateLimiter(times=5, seconds=86400)
        ...
        "messages": [
            {
                ...
                "content": (
                    ...
                    f"- sources: An array of reputable sources as strings (you MUST not include url of the tweet -- linked here {tweet_url}) -- as a source; the result of doing so would be catastrophic. It is of the utmost importance that you provide sources that are reputable and reliable. "
                    f"- factCheck: A search-verified fact check of the tweet. If you cannot find a reputable source to back up the claims made in the tweet, then make that clear. If the tweet doesn't require fact-checking, respond with: "
